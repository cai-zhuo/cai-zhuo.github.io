...
export function normalizeSchema(schema) {
    ...
    // Handle canvas object (extract width, height, backgroundColor to root)
    if (normalized.canvas && typeof normalized.canvas === "object") {
        if (normalized.canvas.width !== undefined) {
            normalized.width = normalized.canvas.width;
        }
        if (normalized.canvas.height !== undefined) {
            normalized.height = normalized.canvas.height;
        }
        // Store backgroundColor if present (could be used for output or further processing)
        if (normalized.canvas.backgroundColor !== undefined) {
            normalized.backgroundColor = normalized.canvas.backgroundColor;
        }
        delete normalized.canvas;
    }
    // Ensure id exists at root
    if (normalized.id === undefined) {
        normalized.id = "canvas";
    }
...
function normalizeChild(child) {
    ...
    if (normalized.type === "shape") {
        ...
        if (Array.isArray(normalized.shapes)) {
            normalized.shapes = normalized.shapes.map((cmd) => {
                ...
                if (c.type === "ellipse") {
                    ...
                    if (c.startAngle === undefined)
                        c.startAngle = 0;
                    if (c.endAngle === undefined)
                        c.endAngle = 2 * Math.PI;

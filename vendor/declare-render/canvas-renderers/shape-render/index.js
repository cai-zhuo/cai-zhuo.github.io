...
export class ShapeRender extends BaseRender {
    constructor(ctx, data) {
        ...
        this.draw = async () => {
            ...
            const drawImpl = () => {
                ...
                let lastAppliedStyle = this.data.style;
    ...
    executeCommand(cmd, offsetX, offsetY) {
        ...
        switch (cmd.type) {
            ...
            case "fillRect":
                if (cmd.rx !== undefined || cmd.ry !== undefined) {
                    ...
                    this.ctx.fillRect(cmd.x + offsetX, cmd.y + offsetY, cmd.width, cmd.height);
                }
                break;
            case "strokeRect":
                if (cmd.rx !== undefined || cmd.ry !== undefined) {
                    ...
                    this.ctx.strokeRect(cmd.x + offsetX, cmd.y + offsetY, cmd.width, cmd.height);
                }
                break;
            case "clearRect":
                this.ctx.clearRect(cmd.x + offsetX, cmd.y + offsetY, cmd.width, cmd.height);
                break;
            case "beginPath":
                this.ctx.beginPath();
                break;
            ...
            case "arc":
                ...
                if (cmd.radiusX !== undefined && cmd.radiusY !== undefined) {
                    ...
                    this.ctx.ellipse(cmd.x + offsetX, cmd.y + offsetY, cmd.radiusX, cmd.radiusY, 0, // rotation (can be added later if needed)
                    cmd.startAngle, cmd.endAngle, cmd.counterclockwise || false);
                }
                else if (cmd.radius !== undefined) {
                    // Circular arc
                    this.ctx.arc(cmd.x + offsetX, cmd.y + offsetY, cmd.radius, cmd.startAngle, cmd.endAngle, cmd.counterclockwise || false);
                }
                else {
                    // Fallback: use radiusX and radiusY if available, or default radius
                    const radius = cmd.radiusX ?? cmd.radiusY ?? 0;
                    this.ctx.arc(cmd.x + offsetX, cmd.y + offsetY, radius, cmd.startAngle, cmd.endAngle, cmd.counterclockwise || false);
                }
                break;
            case "ellipse":
                this.ctx.beginPath(); // Ensure new path for ellipse
                this.ctx.ellipse(cmd.x + offsetX, cmd.y + offsetY, cmd.radiusX, cmd.radiusY, cmd.rotation, cmd.startAngle, cmd.endAngle, cmd.counterclockwise || false);
                break;
            case "arcTo":
                this.ctx.arcTo(cmd.x1 + offsetX, cmd.y1 + offsetY, cmd.x2 + offsetX, cmd.y2 + offsetY, cmd.radius);
                break;
            case "quadraticCurveTo":
                this.ctx.quadraticCurveTo(cmd.cp1x + offsetX, cmd.cp1y + offsetY, cmd.x + offsetX, cmd.y + offsetY);
                break;
            case "bezierCurveTo":
                this.ctx.bezierCurveTo(cmd.cp1x + offsetX, cmd.cp1y + offsetY, cmd.cp2x + offsetX, cmd.cp2y + offsetY, cmd.x + offsetX, cmd.y + offsetY);
                break;
            case "fill":

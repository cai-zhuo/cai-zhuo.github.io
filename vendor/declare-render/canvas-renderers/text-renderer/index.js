import { cloneDeep, groupBy, isNumber, isObject, isUndefined, last, map, values, } from "https://esm.sh/lodash-es@4.17.21";
import { BaseRender } from "../base-renderer.js";
import { highlightLogics } from "./types.js";
import { Highlighter, HighlightType } from "./highlighter.js";
const rightFlow = (opts) => () => opts.reverse().reduce((ret, cur) => cur(ret), undefined);
export class TextRender extends BaseRender {
    constructor(ctx, engine, data, options) {
        super();
        this.lines = [];
        this.hasWarnedNoRenderableText = false;
        this.layout = async () => {
        ...
        this.parseFontSize = () => {
            ...
            if (isObject(fontSize)) {
                const fontObject = fontSize;
                const count = this.data.content.length;
                const { horizonalGap = 0 } = this.data.style;
                const maxWidth = this.data.width;
                this.data.style.fontSize = Math.min(Math.max((maxWidth - (count - 1) * horizonalGap) / count, fontObject.min), fontObject.max);
            }
        };
        this.rotateCanvas = () => {
        ...
        this.metricsLines = () => {
            ...
            if (oneLineMaxWidth <= 0) {
                this.warnNoRenderableText(`skip layout because text width is ${oneLineMaxWidth} (<= 0)`);
                return [];
            }
            ...
            return this.data.content
                ...
                const boundingHeight = (metrics.actualBoundingBoxAscent ?? 0) +
        ...
        this.coordinateLines = (lines) => {
            const { width: containerWidth, height: containerHeight, x: containerStartX, y: containerStartY, } = this.data;
            ...
            return nonEmptyLines.map((mline, lineIndex) => {
                const currentLineWidth = mline.reduce((sum, mc) => sum + mc.metrics.width, 0) +
                ...
                const maxHeightAline = Math.max(...mline.map((c) => c.metrics.actualBoundingBoxAscent ?? 0));
                let Y = (preY =
        ...
        this.drawChars = (lines) => {
            ...
            if (Highlighter.highlightByChar.includes(highlightType)) {
                ...
                lines
                    ...
                    .forEach((c) => {
                    if (!highlight.color)
                        throw new Error("color is required for highlight type:" + highlightType);
                    switch (highlightType) {
                        ...
                        case HighlightType.UNDERLINE:
                            ...
                            break;
                        default:
                            throw new Error(`[Renderer] unknown highlight type with ${highlightType})`);
                    }
                });
                lines
                    .flat()
                    .filter((c) => highlight && indexes.length ? !indexes.includes(c.index) : true)
            ...
            switch (highlight.type) {
                case HighlightType.SVG:
                    values(groupBy(highlightChars, "lineIndex")).forEach((chars) => {
                        ...
                        if (!this.svg)
                            throw new Error("svg url in highlight style is not correct: " + highlight.style);
        ...
        this.drawBackground = (lines) => {
            if (!this.hasRenderableChars(lines))
                return lines;
            if (!this.data.style.backgroundColor)
                return lines;
        ...
        this.getGlyphTightBounds = (lines) => {
            const { horizonalGap = 0, padding } = this.data.style;
            if (!lines)
                throw new Error("can not get text coordinates before layouted");
            if (!this.hasRenderableChars(lines)) {
            ...
            const maxWidth = lines.reduce((maxWidth, line) => Math.max(maxWidth, line.reduce((sum, mc) => sum + mc.metrics.width, 0) +
                horizonalGap * (line.length - 1)), 0);
            ...
            const actualPadding = isUndefined(padding)
                ...
                : isObject(padding)
                    ? padding
                    : { x: padding, y: padding };
            const firstLineTopest = Math.max(...lines[0].map((c) => c.metrics.actualBoundingBoxAscent ?? 0));
